...
from preprocess import dead_mcts_only, labeled_all
...
def normalize_all_mcts():
    all_mcts = labeled_all()
    data_df = list()
    ...
    for mct_name in tqdm(all_mcts):
        for attribute in columns:
            normalized_data = MCT(mct_name).generate_comparison_tuple(attribute)
            data_df.append(normalized_data)
    df = pd.DataFrame(data_df)
    df.to_csv("data/normalized_all_mcts.csv", index=False)
