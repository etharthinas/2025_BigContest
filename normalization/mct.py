...
class MCT:
    ...
    def normalize(self, attribute, by = ["bzn", "zcd", "all"]) -> pd.DataFrame:
        if by == "bzn":
            same = self.merged_df[self.merged_df["HPSN_MCT_BZN_CD_NM"] == self.bzn_cd_nm][[attribute, "TA_YM"]]
        elif by == "zcd":
            same = self.merged_df[self.merged_df["HPSN_MCT_ZCD_NM"] == self.zcd_nm][[attribute, "TA_YM"]]
        elif by == "all":
        ...
        normalized = ((self.get_time_attribute(attribute) - avg) / std).fillna(0) # if std is NaN
        return normalized
