...
def preprocess(DATA_PATH: str):
    ...
    dfs = [
        pd.read_csv(f"{DATA_PATH}/{filename}", encoding="cp949")
        for filename in filenames
    ]
    mct_info, revenue_info, customer_info = dfs

    merged_df = pd.merge(
        revenue_info, customer_info, on=["ENCODED_MCT", "TA_YM"], how="outer"
    )
    ...
    merged_df["MCT_ME_D"] = pd.to_datetime(
        merged_df["MCT_ME_D"].fillna(""), format="%Y%m%d.0", errors="coerce"
    )
...
def dead_mcts_only(DATA_PATH=DATA_PATH):
    ...
    dead_mcts = (
        merged_df[merged_df["MCT_ME_D"].notna()]["ENCODED_MCT"].unique().tolist()
    )
    return dead_mcts
